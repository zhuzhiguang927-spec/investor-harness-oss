...
description: 选股 / 找标的 skill。用于把一个模糊的方向、产业链缺口、景气环节或"涨得少"的想法，拆成可执行的筛选框架、候选池和后续研究顺序。适合做主题挖掘、补短板、二阶受益、低涨幅补涨和观察池扩充。若筛选起点是用户贴出的具体事件、公告、政策、新闻、业绩、产业数据或边际变化材料，或用户问“这件事对哪些公司利好 / 这个业绩怎么看”，必须改用 `event-driven-opportunity`。
inputs:
...
如果用户已经给出具体事件/变化/业绩材料，并要求判断哪些公司受益、哪些票值得看、哪些公司更值得投资，或问这件事对哪些公司利好、这个业绩怎么看，不要继续使用本 skill，改用 `event-driven-opportunity`。
